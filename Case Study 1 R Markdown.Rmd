---
title: "Case Study 1"
author: "Max Pagan and Christian Castro"
date: "2023-10-21"
...
```
## Introduction
In this project, authors Christian Castro and Max Pagan will take a look at data provided to us by Budweiser in order to provide insight about beers, breweries, and all the information about them that we can gather. Our goals for this project are to understand what is contained in the data,analyze basic statistics of the data and draw conclusions about demographics and trends, so that we can recommend future strategies to Budweiser to gain further edge in the competitive beer marketplace. We will do so by answering the nine questions below.
...
ggplot(breweries_per_state, aes(x = reorder(State, -Brewery_Count), y = Brewery_Count)) + 
  ...
  geom_text(aes(label = Brewery_Count), vjust = -0.5) +
...
ggplot(median_ibu_per_state, aes(x = reorder(State, -Median_IBU), y = Median_IBU)) + 
  ...
  geom_text(aes(label = sprintf("%.f", Median_IBU)), vjust = -0.5) +
...
ggplot(median_abv_per_state, aes(x = reorder(State, -Median_ABV), y = Median_ABV)) + 
  geom_bar(stat = "identity", fill = "darkblue") +
  geom_text(aes(label = sprintf("%.1f%%", 100*Median_ABV)), vjust = -0.5) +
